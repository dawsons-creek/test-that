...
class TestResult:
    ...
    def __init__(self, name: str, passed: bool, error: Optional[Exception] = None, 
                 duration: float = 0.0):
        self.name = name
        self.passed = passed
        self.error = error
...
class TestRunner:
    ...
    def run_test(self, test_name: str, test_func: Callable,
                 setup_result: Any = None) -> TestResult:
        """Run a single test function."""
        start_time = time.time()

        try:
            # Check if test function expects setup result
    ...
    def run_suite(self, suite: TestSuite) -> List[TestResult]:
            ...
            try:
                ...
                for test_name, _ in suite.tests:
                    results.append(TestResult(
                        test_name, False, 
                        error=Exception(f"Setup failed: {e}")
                    ))
                return results
        ...
        if suite.teardown_func:
            try:
                ...
                # Teardown failure doesn't affect test results, but we should log it
                if self.verbose:
